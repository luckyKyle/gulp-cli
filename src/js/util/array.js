...
/**
 * Usage: 返回两个数组中存在的元素的列表。
 * “从b创建Set , 然后使用Array.filter() on a只保留b中包含的值.”
 * @param {需要传入的一数组} a
 * @param {需要传入的二数组} b
 * Example: intersection([1,2,3], [4,3,2]) -> [2,3]
 */
export const intersection = (a, b) => { const s = new Set(b); return a.filter(x => s.has(x)) }
